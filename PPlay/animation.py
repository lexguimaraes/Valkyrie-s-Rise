...
class Animation(gameimage.GameImage):
    ...
    def draw(self,flip = False,sprite = False):
        if(self.drawable):
            # Clips the frame (rect on the image)
            if flip == False or sprite:
                ...
                clip_rect = Rect(abs(self.curr_frame-self.final_frame)*self.width,
                                 0,
                                 self.width,
                                 self.height)

            window.Window.get_screen().blit(
                pygame.transform.flip(self.image, flip, False),
